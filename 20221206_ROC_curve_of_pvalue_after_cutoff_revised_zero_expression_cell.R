...
c.result <-read.table("summary_of_correlaiton_between_RBP_level_and_miRNA_level_with_excluding_outlier_and_considering_no_expression.txt",sep="\t",header = T,stringsAsFactors = F)
...
merge.df[!is.na(merge.df[,6]),8] <-"match"
merge.df[is.na(merge.df[,6]),8] <-"not_match"
colnames(merge.df)[8] <-"match_with_CCLE"
merge.df[,6] <-ifelse(is.na(merge.df[,6]),0,merge.df[,6])

# check number of combinations with/without physical interaction
# 455(unique:431) treiber's combinations with physical interaction(score>3) are matched with CCLE correlation analysis result
# 5761(unique:4420) treiber's combinations without physical interaction(score<=3) are matched with CCLE correlation analysis result
phy <-merge.df[merge.df[,6]>3&merge.df[,8]=="match",1]
length(unique(phy))

no.phy <-merge.df[merge.df[,6]<=3&merge.df[,8]=='match',1]
...
for (i in 1:length(cutoff)) {
  ...
  result[result[,6]>3,9] <-1
  result[result[,6]<=3,9] <-0
  colnames(result)[9] <-"physical_interaction"
  ...
  count <-as.data.frame(table(result[,8],result[,9]))
  p <-count[3,3]
  np <-count[1,3]
  
  # draw ROC curve
  pdf(paste0("ROC_curve_pvalue_cutoff_",cutoff[i],"_zero_expression.pdf"))
